...
int *stringify_matrix(int **matrix, int rows, int columns, int *error) {
    int *matrix_string = *error == 0 ? malloc(rows * columns * sizeof(int)) : NULL;
...
void sort_vertical(int **matrix, int rows, int columns, int **result, int *error) {
...
void sort_horizontal(int **matrix, int rows, int columns, int **result, int *error) {
    int *matrix_string = stringify_matrix(matrix, rows, columns, error);
    if (0 == *error) {
        int string_size = rows * columns;
        qsort(matrix_string, string_size, sizeof(int), matrix_elem_compare);
