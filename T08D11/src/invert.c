...
double **invert(double **matrix, int rows, int columns, int *error) {
    ...
    if (*error || fabs(determinant) < EPSILON) {
        *error = 1;
        ...
        for (int i = 0; i < rows && 0 == *error; i++) {
            for (int j = 0; j < columns && 0 == *error; j++) {
                inverse_matrix[i][j] = findCofactor(matrix, rows, columns, i, j, error) / determinant;
            }
        }
...
double findCofactor(double **matrix, int rows_amount, int columns_amount, int row, int column, int *error) {
